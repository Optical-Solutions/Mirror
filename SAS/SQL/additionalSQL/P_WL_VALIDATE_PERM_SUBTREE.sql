--------------------------------------------------------
--  DDL for Procedure P_WL_VALIDATE_PERM_SUBTREE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "MAXDATA"."P_WL_VALIDATE_PERM_SUBTREE" (
...
/*----------------------------------------------------------------------------
$Log: 2362_p_wl_validate_perm_subtree.sql,v $
Revision 1.8  2007/06/19 14:38:54  clapper
FIXID AUTOPUSH: SOS 1238247

Revision 1.4  2006/06/05 19:50:27  anchan
...
Revision 1.2  2006/04/20 14:49:45  anchan
Fixed typo

Revision 1.1  2006/04/13 13:26:28  anchan
FIXID : Pushed out the bulk of the validation code into two sub-procs from p_wl_validate_object
...
t_proc_name        	VARCHAR2(32) 		:= 'p_wl_validate_perm_subtree ';
...
WHEN 1 THEN
	...
	n_sqlnum := 2200;
	SELECT COUNT(*) INTO t_rowcount
	FROM maxdata.wlw1_worksheet_task w1
	WHERE worksheet_template_id=in_template_id
	AND NOT EXISTS
	...
	BEGIN
		maxdata.p_wl_validate_perm_kpi_pa(in_object_type_cd,in_template_id,in_object_no,out_complete_flg);
	END;
	END IF;
...
WHEN 4 THEN
	...
	IF (t_rowcount=0) THEN
		RAISE_APPLICATION_ERROR (-20001, '[WL_NO_KPISET] Spefied object does not exist: kpi_set_no='||in_object_no);
	END IF;
