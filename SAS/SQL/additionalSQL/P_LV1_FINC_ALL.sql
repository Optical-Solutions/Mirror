--------------------------------------------------------
--  DDL for Procedure P_LV1_FINC_ALL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "MAXDATA"."P_LV1_FINC_ALL" 
as
...
DELETE FROM maxdata.lv1finc;
Commit;
insert into maxdata.lv1finc
        (last_update,
         changed_by_batch,
         period,
         days_in_period,
...
group by lv4loc.lv1loc_id,
         lv4finc.period,
         lv4finc.cycle;

--Added by Maurice to populate period_start_date on 02/12/2001
Update maxdata.lv1finc set period_start_date
= ( select period_start_date from maxapp.period_lkup
where period_id = lv1finc.period and cycle_id = lv1finc.cycle
and period_type = t_period_type);

commit;

--This update is not required as these functionality is taken care in DM Task 38
--select max(Cycle * 100 + Period) INTO iCyclePdID from maxdata.lv10finc;

--Update maxapp.mmax_config
--set Current_cycle = iCyclePdID/100, current_period = mod(iCyclePdID, 100);
...
/
  ...
  GRANT EXECUTE ON "MAXDATA"."P_LV1_FINC_ALL" TO "MADMAX";
  GRANT EXECUTE ON "MAXDATA"."P_LV1_FINC_ALL" TO "MAXUSER";
