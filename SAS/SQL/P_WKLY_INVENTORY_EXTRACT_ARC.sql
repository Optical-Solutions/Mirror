--------------------------------------------------------
--  DDL for Procedure P_WKLY_INVENTORY_EXTRACT_ARC
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ERIC"."P_WKLY_INVENTORY_EXTRACT_ARC" 
Is 
...
(P_Process_Id, 20,'Step 2:Extract Data from SASPRD inv_move_arc:p_wkly_inventory_extract', 
...
execute immediate 'Insert /*+ Append */ Into Wkly_Inv_Move_Extract
      Select /*+ FULL(I) PARALLEL(I,8) */ 
       ...
       I.Retail_Price, Retail_Price_Final, I.Landed_Unit_Cost, I.Average_Cost,
       i.Merchandising_Year, i.Merchandising_Week, I.section_id
 From   inv_move_arc I ' || v_where;
 Commit;
...
END P_WKLY_INVENTORY_EXTRACT_ARC;
