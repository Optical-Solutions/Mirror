...
Readonly my $g_logfile => '/usr/local/mccs/log/mcl/'
  . basename(__FILE__) . '.log';

my $crt_mclane_data = <<CRTMCLRPTFILE;
Begin MRI_BUILD_MCLANE_DATA_RCV_2(:NumOfDays); End;
CRTMCLRPTFILE

my $stage;
my $retry;
my $g_weekEndingDate;
#use constant DEFAULT_OUTPUT_DIR => '/usr/local/mccs/data/mcl/';
Readonly my $DEFAULT_OUTPUT_DIR => '/usr/local/mccs/data/mcl/';
...
# Retrieve Number of days to extract from Cognos DB
...
# Extract Subject line verbiage for negative email
my $g_email_sub = $g_cfg->mcl_DATA->{email_sub};

# Extract file name to produce for MCL from configuration
( my $g_mclFile = $g_cfg->mcl_DATA->{FILE_TO_PRODUCE} ) =~
  s/YYYYMMDD/my $x = `date +%Y%m%d`; chomp $x; $x/e;
...
# Add on 'where' clause if list of sites contains at least 1 site
	...
	my $OUT = IO::File->new( $outfile, "w") or die "Could not write $g_mclFile";
