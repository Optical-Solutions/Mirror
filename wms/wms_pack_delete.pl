...
my $g_object_logfile  = '/usr/local/wms/log/' . basename(__FILE__) . '_Query.log';
...
my $go_wms_ba = MCCS::WMS::Query->new(
                                       dbname  => $g_dbname,
                                       outfile => 'PACK_DELETE' #, logfile => $g_object_logfile
                                     );
...
where  
  ...
  and
       ...
       -- ADD and DELETE type
            ...
            and     trunc(zz.operation_date)    =  trunc(sysdate) -- TODO
            group by 
