...
## connect to db:
$invoice_obj->{'dbh'} = $invoice_obj->_db_connect();
...
$invoice_obj->{'log_obj'}->log_info("Start time: $t_suffix\t $0\n");
 ...
 my $invoice_id = '14';
 ##my ($ret_ref1, $ret_ref2, $ret_ref3) = $invoice_obj->load_obj_from_db($invoice_id);

## print Dumper($ret_ref1);

my ($ret1, $ret2) = $invoice_obj-> select_from_v_edi_summary_old();
print Dumper($ret1);
print Dumper($ret2);

my ($ret3, $ret4) = $invoice_obj->second_select_from_v_edi_summary();
