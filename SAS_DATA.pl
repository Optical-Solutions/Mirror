...
my $DEFAULT_OUTPUT_DIR_PRCLS = '/usr/local/mccs/data/sas/pre_reclass/';
...
my $debug;
my $database;
my $log_dir="/usr/local/mccs/log/";
my $log = $log_dir . "sas_data_$$.log";
my $merchyear;
my $merchweek;
my $merchweek2;
my $help;
...
make_dir($DEFAULT_OUTPUT_DIR);
make_dir($DEFAULT_OUTPUT_DIR_PRCLS);
make_dir($log_dir);
my $badmsg;
