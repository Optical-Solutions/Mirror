...
# Date: Fri Dec  8 11:07:38 EST 2023
# Desc:
#
#----------------------------------------------------------------
use strict;
...
## check if the process has been run already:
## if not yet, run it, 
## else, exit

my $t_dir = '/usr/local/mccs/data/mercurygate_tracking';
$tms->{'log_obj'}->info("Ymd string: $ymd_str");

my @track_files =  glob ("$t_dir/$ymd_str*");

if (@track_files > 0){ 
    my $buff = Dumper(\@track_files);
    $tms->{'log_obj'}->info("File on $ymd_str is found. Program will exit...");
    $tms->{'log_obj'}->info($buff);
    my $end_time = strftime( '%Y-%m-%d %H:%M:%S', localtime );
    $tms->{'log_obj'}->info($end_time);
    exit();
}else{
    $tms->{'log_obj'}->info("NO file on $ymd_str is found. Program will proceed..");
}
