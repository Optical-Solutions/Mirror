...
my $remote_path   = "/usr/local/mccs/data/incoming/altria";
my $g_count       = "";
...
sub push_to_altria {
    ...
    my $username      = $secret->{username} // "N/A";
    my $password      = $secret->{password} // "N/A";
...
# #########
 ...
 my @files=$sftp->ls("/usr/local/mccs/data/incoming");
 print ("printing file names\n");
 foreach my $f(@files) {
 my $fname = $f->{fname};
    print "$fname\n";
 }
 if (copy($local_path,$remote_path)) {
...
print ("value of sftp server $sftp_server\n" );

    # Check if the remote directory exists, if not, create it
    unless ($sftp->stat($remote_path)) {
        $sftp->mkdir($remote_path) or die "Couldn't create directory: " . $sftp->error;
        print "Directory created: $remote_path\n";
    }

    # Upload the local file
#     if ($sftp->put($local_path, $remote_path)) {
#         print "File uploaded successfully.\n";
#     } else {
#         die "File upload failed: " . $sftp->error;
#     }
#   $sftp->remove($remote_path . "/" . $file ) or die "couldnt remove file $file\n";
