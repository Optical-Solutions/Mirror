...
my $param_dow_plus_7 = defined($g_opt{w}) ? $g_opt{w} : undef;
my $param_dow_plus_1 = defined($g_opt{x}) ? $g_opt{x} : undef;
...
SA.REGISTER_ID 
   ...
   my $sql_sub = eval "defined($param_dow_plus_7) ? $param_dow_plus_7 : '($g_day_of_week + 7)'";
   $sql =~ s/dow\+7/$sql_sub/eg;

   $sql_sub = eval "defined($param_dow_plus_1) ? $param_dow_plus_1 : '($g_day_of_week + 1)'";  
   $sql =~ s/dow\+1/$sql_sub/eg; 

   # Change this to the actual SQL

   $sql =~ s/\$g_day_of_week/$g_day_of_week/g;
...
WHERE               S.BUSINESS_UNIT_iD = 30 AND
   ...
   $sql =~ s/dow\+7/defined($param_dow_plus_7) ? $param_dow_plus_7 : '($g_day_of_week + 7)'/eg;
   $sql =~ s/dow\+1/defined($param_dow_plus_1) ? $param_dow_plus_1 : '($g_day_of_week + 1)'/eg;
   $sql =~ s/\$g_day_of_week/$g_day_of_week/g;
