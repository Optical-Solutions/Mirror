...
select              '64549' ||'|'||
                    (select TO_CHAR(m.week_ending_date,'YYYYMMDD') from MERCH_CAL_MIKE_WEEK M where Sa.SALE_DATE BETWEEN M.WEEK_STARTING_DATE AND M.WEEK_ENDING_DATE )||'|'||
                    ...
                    SD.BAR_CODE_ID||'|'||
                    S.DESCRIPTION||'|'||
                    CASE WHEN V.CLASS_ID = '1100' AND STYLE_TYPE = 'MULTI' THEN 'CARTON'
                         WHEN V.CLASS_ID = '1100' AND STYLE_TYPE = 'SINGLE' THEN 'PACK'
                         WHEN V.CLASS_ID = '1200' AND STYLE_TYPE = 'MULTI' THEN 'SLEEVE'
                         WHEN V.CLASS_ID = '1200' AND STYLE_TYPE = 'SINGLE' THEN 'CAN'
                         ELSE 'EA'
                    END ||'|'||
                    SD.QTY||'|'||
...
join  SALE_DETAILS SD on (
                          ...
                          SA.REGISTER_ID = SD.REGISTER_ID )
                      
join STYLES S on (s.business_unit_id = sa.business_unit_id and
...
join CHARACTERISTIC_VALUES cv on (Sc.business_unit_id = cv.business_unit_id and
                                  ...
                                  SC.CHARACTERISTIC_VALUE_ID = CV.CHARACTERISTIC_VALUE_ID)
...
sub push_to_altria {
    ...
    my $sftpHash      = $util->get_secret($token); 
    print "DEBUG: Full Secret Hash:\n" . Dumper($sftpHash) . "\n";

    if ($sftpHash && (exists $sftpHash->{user} || exists $sftpHash->{username})) {
        print "DEBUG: Username: $sftpHash->{user}\n";
    } else {
...
print "DEBUG: Hostname: $sftpHash->{host}\n"; 
print "DEBUG: Username: $sftpHash->{user}\n";  
print "DEBUG: Username: $sftpHash->{pw}\n"; 
# Server and authentication details
 my $hostname      = $sftpHash->{host};
 #print "hostname is $hostname \n";
 my $username      = $sftpHash->{username} // "N/A";
 my $password      = $sftpHash->{password} // "N/A";
 my $local_path;
 my $remote_path;
 my $sftp;

 # Local file path and remote destination
  $local_path =   $g_txt_d . '/' . $file;
  $remote_path = $g_cfg->Altria->{sftp_dir} . '/' . $file;
 #print sftp_dir;

#TODO remove next 5 lines.
#Testing for configuration values, next four lines.
# print "Host is " . $hostname . "\n";
# print "User is " . (defined $username ? $username : "MISSING!"). "\n";
# print "pass is " . (defined $password ? "set (hidden)" : "MISSING!"). "\n";
# print "Password is " . $password . "\n";
# #########

 # Create an SFTP object
  $sftp = Net::SFTP::Foreign->new($hostname, user => $username, password => $password);


 # Check for errors in creating the SFTP object
 croak "Could not establish SFTP connection: " . $sftp->error unless defined $sftp;

 # Upload the local file
  $sftp->put($local_path, $remote_path) or croak "Failed to upload file: " . $sftp->error;
