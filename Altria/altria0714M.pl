...
my $test_to_email   = $ENV{'TEST_EMAIL_RECIPIENT'} || 'kaveh.sari@usmc-mccs.org';
...
# my $g_emails      = $g_cfg->Altria->{tech_emails};
 my $g_emails={'kaveh' => $test_to_email};
# my $g_cust_emails = $g_cfg->Altria->{customer_email};
my $g_cust_emails = {'kaveh2' => 'ksari2000@gmail.com'};
GetOptions(
    'd'   => \$DEBUG,
...
sub send_notify_email { 
   ...
   eval {
      ...
      my $all_emails = { %{$g_emails}, %{$g_cust_emails} };
      ...
      if ($mailer_simple->send_mail()) {
         print "Altria Simple email allegedly sent successfully!\n";
      } else {
         print "Altria Failed to send simple email (as per return code).\n";
      }
   };
   if ($@) {
      my $error = $@;
      eval { $error = decode('UTF-8', $error, Encode::FB_WARN) };
      print "Altria DIED while trying to send simple email: $error\n";
   }
...
eval { my_main() };
if ($@) {
    send_notify_email( "ERROR on " . __FILE__ . ' ' . $g_long_date,
        "Untrapped Error:\n\n", " $@" );
    print ($@);
1;
